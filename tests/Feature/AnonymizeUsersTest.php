...
test('it anonymizes graduated stockholmscience users in July', function () {
    ...
    $this->artisan('app:anonymize-users');
...
test('it does not anonymize graduated stockholmscience users outside of July', function () {
    ...
    expect($graduated->refresh()->isAnonymized())->toBeFalse();
});
...
test('it sends warnings to inactive users with other emails after 6 months', function () {
    ...
    Mail::assertSent(InactivityWarningMail::class, function ($mail) {
...
test('it resets inactivity_warning_sent_at when user becomes active', function () {
    ...
    expect($user->last_activity_at->isToday())->toBeTrue();
});
